...
| Local layout overrides | Project-specific partials, render hooks, shortcodes | `layouts/` |
| Asset pipeline (CSS extensions) | CSS injected/extended on top of theme via Hugo's asset pipeline | `assets/css/extended/` |
| Static assets | Images, presentations, raw CSS, generated CV files | `static/` |
| Data files | Structured data consumed by scripts and templates | `data/skills.toml` |
...
- No server-side runtime: all output is fully static; the only runtime "code" in the browser is third-party CDN scripts (KaTeX for math, Twitter widgets for the `x` shortcode).
...
- Location: `data/`
- Contains: `skills.toml` (categorized skill lists used by the CV generator).
- Depends on: nothing.
- Used by: `scripts/generate_cv.py` (and any Hugo template via `.Site.Data.skills`, though no template currently does).
...
- Contains: Partials (`extend_head.html`, `extended_head.html`, `math.html`), shortcodes (`x.html`), markup render hooks (`_default/_markup/render-link.html`).
- Depends on: theme defaults; content frontmatter (`.Params.math` toggles KaTeX include).
- Used by: Hugo at build time.

**Asset / styling layer:**
- Purpose: CSS, images, fonts, prebuilt static files.
- Location: `assets/css/extended/` (Hugo asset pipeline; theme picks these up to extend its base CSS) and `static/` (copied verbatim into `public/`).
- Contains: `assets/css/extended/center-images.css`; `static/terminal.css`, `static/css/{custom,cv,syntax}.css`, `static/images/`, `static/presentations/`.
- Depends on: theme conventions for `assets/css/extended/`.
...
- Examples: `assets/css/extended/center-images.css`.
- Pattern: Drop `.css` files into `assets/css/extended/`; the terminal theme concatenates them after its own styles.

## Entry Points

**Hugo build:**
- Location: `hugo.toml` (config), `content/_index.md` (homepage content), `themes/terminal/layouts/index.html` (homepage template via the `layout = "index"` frontmatter).
